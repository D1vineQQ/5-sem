...
int main() {
    ...
    string pipeName = "\\\\N1ce\\pipe\\Tube";  //  L"\\\\.\\pipe\\Tube"; 
    LPCWSTR pipePath = std::wstring(pipeName.begin(), pipeName.end()).c_str(); // Преобразуем в LPCWSTR
    ...
    // Отправка сообщения нулевой длины для завершения сервера
    WriteFile(hPipe, "", 0, NULL, NULL);
