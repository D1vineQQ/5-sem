...
void main()
{
    DWORD wb;
    HANDLE mailClient;
    vector<string> serverPaths = {
        //R"(\\.\mailslot\Box)"
        R"(\\ApathyIsDeath\mailslot\Box)"
    };
    ...
    try
    {
        for (const auto& serverPath : serverPaths)
        {
            wstring wServerPath = wstring(serverPath.begin(), serverPath.end());

            if ((mailClient = CreateFile(wServerPath.c_str(), GENERIC_WRITE, FILE_SHARE_READ, NULL, OPEN_EXISTING, NULL, NULL)) == INVALID_HANDLE_VALUE)
            {
            ...
            {
                wbuf = "Hello from Mailslot-client " + to_string(i);
            ...
            // Отправляем пустое сообщение как сигнал завершения
