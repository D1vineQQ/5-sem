...
bool PutAnswerToClient(char* message, const struct sockaddr_in* to, int* lto)
{
    ...
    int sent_len;
    if ((sent_len = sendto(sS, message, strlen(message), 0, (sockaddr*)to, *lto)) == SOCKET_ERROR) {
    ...
    try
    {
        ...
        while (true) {
            if (GetRequestFromClient(name, port, (sockaddr*)&client_addr, &addr_len))
            {
                ...
                // Отправляем сообщение "Hi, Bro!" клиенту
                if (!PutAnswerToClient((char*)"Hi, Bro!", &client_addr, &addr_len))
                {
